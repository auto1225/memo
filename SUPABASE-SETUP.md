...
3. **Redirect URLs**에 추가:
   ...
   https://justanotepad.com/v2
   https://justanotepad.com/v2/
   https://justanotepad.vercel.app/app
   https://justanotepad.vercel.app/v2
   https://justanotepad.vercel.app/v2/
   http://localhost:*/app
   http://localhost:*/v2
   http://localhost:*/v2/
...
v2에서는 **설정 → Supabase 클라우드 동기화**에서 확인:
- `기본 Supabase 프로젝트 연결됨` 표시
- **Google 로그인** 클릭
- 로그인 후 **지금 동기화** 또는 **자동 동기화** 활성화
- 다른 기기에서도 로그인하면 같은 `user_data` snapshot으로 동기화
...
**Q. 로그인 후 리다이렉트 실패**
→ Supabase "Redirect URLs"에 `/app` 또는 `/v2` 경로 누락된 경우
